...
API_TIMEOUT=2
#MINER_API_PORT=3334
. /hive/miners/custom/quai/h-manifest.conf
local LOG_NAME="$CUSTOM_LOG_BASENAME.log"

stats_raw=`echo '{"id":0,"jsonrpc":"2.0","method":"miner_getstat1"}' | nc -w $API_TIMEOUT localhost $API_PORT | jq '.result'`
if [[ $? -ne 0  || -z $stats_raw ]]; then
	echo -e "${YELLOW}Failed to read $miner stats_raw from localhost:$API_PORT${NOCOLOR}"
else
	...
	[[ $ETHMINER_FORK == "olminer" ]]    && algo="olhash"

        [[ $(cat /run/hive/miners/ethminer/ethminer.conf | grep -E "\-\-etchash|\-\-ecip1099") ]] && algo="propowz"

	local stats_detail=`echo '{"id":0,"jsonrpc":"2.0","method":"miner_getstatdetail"}' | nc -w $API_TIMEOUT localhost $API_PORT | jq '.result'`
	...
	echo $stats >> /hive/miners/custom/quai/testlog.log
fi
